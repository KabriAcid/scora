...
const FormBadge = ({ result, index }: { result: "W" | "D" | "L"; index: number }) => {
  const variants = {
...
const PositionIndicator = ({ position }: { position: number }) => {
  if (position <= 4) {
    return (
      <div className="w-1 h-8 rounded-full bg-success absolute left-0" />
    );
  }
  if (position >= 18) {
    return (
              ...
              <div className="space-y-1">
                {standings.map((entry, index) => (
                  <motion.div
                    ...
                    whileHover={{ scale: 1.01, backgroundColor: "hsl(var(--accent)/0.1)" }}
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      />
                      <span className="text-sm font-medium truncate">{entry.team.responsiveName}</span>
                    </div>

                    {/* W D L */}
                    <span className="text-xs text-center text-success font-medium">{entry.win}</span>
                    <span className="text-xs text-center text-muted-foreground font-medium">{entry.draw}</span>
                    <span className="text-xs text-center text-destructive font-medium">{entry.loss}</span>
                ...
                <Card className="p-4 bg-card/50 border-border/50">
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    <div className="text-center">
                      <div className="text-2xl font-bold text-primary">{standings[0]?.points}</div>
                      <div className="text-xs text-muted-foreground">Leader Points</div>
                    </div>
                    <div className="text-center">
                      <div className="text-2xl font-bold text-success">{standings[0]?.gf}</div>
                      <div className="text-xs text-muted-foreground">Top Goals</div>
                    </div>
                    <div className="text-center">
                      <div className="text-2xl font-bold text-accent">{standings.reduce((acc, s) => acc + s.played, 0)}</div>
