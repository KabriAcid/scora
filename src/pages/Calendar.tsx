    ...
    <div className="mx-4 min-w-[70px] text-center">
      {match.status === "finished" ? (
        ...
        >
          <span className={cn(
            "text-lg font-bold",
            match.homeScore! > match.awayScore! ? "text-success" : "text-foreground"
          )}>
            {match.homeScore}
          </span>
          ...
          <span className={cn(
            "text-lg font-bold",
            match.awayScore! > match.homeScore! ? "text-success" : "text-foreground"
          )}>
            {match.awayScore}
          </span>
        </motion.div>
...
const CalendarPage = () => {
  ...
  const [currentDateRange, setCurrentDateRange] = useState("Sat 2 Nov - Mon 4 Nov");
        ...
        <div className="p-4">
          ...
          <div className="flex items-center justify-center gap-6 mt-4 text-xs text-muted-foreground">
            <div className="flex items-center gap-1">
              ...
              <span>{matchGroups.reduce((acc, g) => acc + g.matches.filter(m => m.status === "finished").length, 0)} Completed</span>
            </div>
            <div className="flex items-center gap-1">
              <div className="w-2 h-2 rounded-full bg-primary" />
              <span>{matchGroups.reduce((acc, g) => acc + g.matches.filter(m => m.status === "scheduled").length, 0)} Upcoming</span>
            </div>
          </div>
        </div>
      </motion.header>
              ...
              {matchGroups.map((group, groupIndex) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-2 mb-2 ml-1">
                    ...
                    <h2 className="font-bold text-sm text-foreground">{group.date}</h2>
                  </div>
