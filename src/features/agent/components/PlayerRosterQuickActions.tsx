...
const findPlayerByName = (players: Player[], search: string) => {
...
export const PlayerRosterQuickActions = ({
	...
	const handleQuickAction = (player: Player, eventType: string) => {
		...
		if (eventType === "goal") {
			const assistInput = window.prompt(
				`Optional assist for ${player.name} (${activeTeam}). Leave blank if none.`,
			);

			if (assistInput !== null) {
				const assistValue = assistInput.trim();
				if (assistValue) {
					const assistMatch = findPlayerByName(
						[...teamLineup.lineup, ...teamLineup.substitutes],
						assistValue,
					);

					assistPlayer = assistMatch?.name;
					assistPlayerId = assistMatch?.id;
				}
			}
		}
