...
export const EventTimeline = ({
  ...
  const timelineItems = useMemo((): TimelineItem[] => {
    // Photos are managed separately in MatchPhotoCapture — exclude from timeline
    const matchEvents = events.filter((e) => e.type !== "photo");
    const showSeparator =
    ...
    if (!showSeparator || matchEvents.length === 0) return matchEvents;

    const separator: SeparatorItem = {
      isSeparator: true,
      id: "half-separator",
    };
    const firstHalfIndex = matchEvents.findIndex(
      (e) => (e.half ?? "first") === "first",
    );

    if (firstHalfIndex === -1) {
      return [...matchEvents, separator];
    }
    const result: TimelineItem[] = [...matchEvents];
    result.splice(firstHalfIndex, 0, separator);
    return result;
  }, [events, matchPhase]);

  if (!events || timelineItems.length === 0) {
    return (
