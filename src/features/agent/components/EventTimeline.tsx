...
import { Activity, Trash2, Camera } from "lucide-react";
...
export const EventTimeline = ({
  ...
  return (
    <>
                  ...
                  >
                    ...
                    <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-20">
                      <button
                        ...
                        className={`flex items-center justify-center w-10 h-10 rounded-full bg-background border-2 shadow-lg transition-colors ${readOnly
                      ...
                      >
                        {event.type === "photo" ? (
                          <Camera className="w-5 h-5 text-primary pointer-events-none" />
                        ) : (
                          <img
                      ...
                      <div className="w-1/2 flex justify-end pr-10">
                        {event.type === "photo" ? (
                          <div className="space-y-1 text-right">
                            ...
                            <button
                              onClick={() => setPhotoDialogUrl(event.photoUrl ?? null)}
                              className="mt-0.5 block"
                            >
                              <img
                                src={event.photoUrl}
                                alt="match photo"
                                className="w-16 h-12 object-cover rounded-lg border border-border hover:border-primary/60 transition-colors"
                              />
                            </button>
                          </div>
                        ) : (
      ...
      <Dialog open={!!photoDialogUrl} onOpenChange={() => setPhotoDialogUrl(null)}>
        <DialogContent className="max-w-lg p-2 bg-black border-border">
          {photoDialogUrl && (
            <img
              src={photoDialogUrl}
              alt="match photo full view"
